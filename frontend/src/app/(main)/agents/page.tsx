...
export default function TTBAgentsPage() {
  ...
  const loadActivity = useCallback(async () => {
    try {
      ...
      console.error("[Agents] Failed to fetch agent activity", err);
    }
  }, []);
  ...
  return (
    <>
      <Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Review Agents" }]} />
      ...
      <div id="agents-main" style={{ maxWidth: 1200, margin: "0 auto", padding: "24px" }}>
        ...
        >
          {[
            ...
            {
              n: Math.round(agents.reduce((s, a) => s + a.stats.avgTimeSeconds, 0) / agents.length) + "s",
        ...
        <div id="agents-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          {agents.map((agent) => {
            ...
            const approvalRate =
              agent.stats.reviewed > 0 ? Math.round((agent.stats.approved / agent.stats.reviewed) * 100) : 0;
            return (
              <div
                key={agent.id}
                style={{
                ...
                <div style={{ padding: "12px 20px" }}>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <div style={{ height: 6, background: C.lightGray, borderRadius: 3, overflow: "hidden" }}>
                      <div
                        style={{ height: "100%", width: `${approvalRate}%`, background: C.green, borderRadius: 3 }}
                      />
                    </div>
                  </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span style={{ fontSize: 12, color: C.darkGray, flex: 1 }}>
                              {a.submissionId ? (
                                <Link
                                  href={`/queue/${a.submissionId}`}
                                  style={{ color: C.lightBlue, textDecoration: "none" }}
                                >
                                  {a.productName}
                                </Link>
                              ) : (
